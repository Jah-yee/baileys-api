 ...
 */
export const getAlternateJidFromDb = Effect.fn("WhatsAppUtils.getAlternateJidFromDb")(
	function* (jid, maybeClient) {
		...
		const payload = jids.map((j) => {
			const kind: Contact["idType"] = isPhoneNumberJid(j) ? "phone-number" : "lid";
			return { id: j, idType: kind };
		});
