...
export const chats = pgTable(
	"chats",
	{
		...
		lastMsgTimestamp: bigint({ mode: "number" }),
		unreadCount: integer(),
		readOnly: boolean(),
		endOfHistoryTransfer: boolean(),
		ephemeralExpiration: integer(),
		ephemeralSettingTimestamp: bigint({ mode: "number" }),
		...
		tcToken: bytea(),
		tcTokenTimestamp: bigint({ mode: "number" }),
		contactPrimaryIdentityKey: bytea(),
		pinned: integer(),
		muteEndTime: bigint({ mode: "number" }),
		wallpaper: jsonb().$type<proto.IWallpaperSettings>(),
		mediaVisibility: smallint(),
		tcTokenSenderTimestamp: bigint({ mode: "number" }),
		suspended: boolean(),
		terminated: boolean(),
		...
		limitSharingSettingTimestamp: bigint({ mode: "number" }),
...
export const messages = pgTable(
	"messages",
	{
		...
		messageTimestamp: bigint({ mode: "number" }),
		status: smallint(),
		participant: varchar({ length: 128 }),
		messageC2STimestamp: bigint({ mode: "number" }),
		ignore: boolean(),
		starred: boolean(),
		broadcast: boolean(),
		...
		ephemeralStartTimestamp: bigint({ mode: "number" }),
		...
		newsletterServerId: bigint({ mode: "number" }),
		...
		revokeMessageTimestamp: bigint({ mode: "number" }),
	},
